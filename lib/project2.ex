defmodule Project2 do
  ...
  #getting the commandline arguements
    ...
    maxCount=10
    case topology do
      "LINE" -> line(numNodes, algorithm, nil,firstNodeNo, nodeList, maxCount,failureCount)
      "2DGRID" -> imperfect2DGrid(topology,:math.ceil(:math.sqrt(numNodes))*:math.ceil(:math.sqrt(numNodes)), algorithm,1,maxCount,0,0,%{},nodeList,failureCount)
      "IMPERFECT2DGRID" -> imperfect2DGrid(topology,:math.ceil(:math.sqrt(numNodes))*:math.ceil(:math.sqrt(numNodes)), algorithm,1,maxCount,0,0,%{},nodeList,failureCount)
      "FULLNETWORK" -> fullNetwork(numNodes, algorithm,nodeList,firstNodeNo,maxCount,failureCount)
    end
  ...
  def line(numNodes, algorithm, left, nodeNo, list,maxCount,failureCount) when nodeNo<=numNodes do
    ...
    line(numNodes, algorithm,current,nodeNo+1, [current|list],maxCount,failureCount)
  end

  def line(numNodes, algorithm, left, nodeNo, list,maxCount,failureCount) when nodeNo>numNodes do
    if(failureCount>0) do
      killSomeRandomProcesses(failureCount,list)
      IO.puts to_string(:os.system_time(:millisecond))<>"\t"<>to_string(failureCount)<>" nodes killed after creating topology"
    end
    if String.equivalent?(algorithm,"GOSSIP") do
      pehla=Enum.random(list)
      IO.puts to_string(:os.system_time(:millisecond))<>"\tNode: "<>(List.to_string(:erlang.pid_to_list(pehla)) <> "\thas has started spreading the gossip")
      Project2.LineServer.sendGossip(pehla, "Abra ka dabra")
      unlimitedLoop(list) 
    else
      if String.equivalent?(algorithm,"PUSH-SUM") do
        pehla=Enum.random(list)
        IO.puts to_string(:os.system_time(:millisecond))<>"\tNode: "<>(List.to_string(:erlang.pid_to_list(pehla)) <> "\thas started push sum")
  ...
  def fullNetwork(numNodes, algorithm,list,nodeNo,maxCount,failureCount) when nodeNo<=numNodes do
    {:ok, current}=Project2.FullNetworkServer.start_link(nodeNo,maxCount)
    Project2.FullNetworkServer.setCurrent(current)
    fullNetwork(numNodes, algorithm,[current|list],nodeNo+1,maxCount,failureCount)
  end
  def fullNetwork(numNodes, algorithm,list,nodeNo,maxCount,failureCount) when nodeNo>numNodes do
    for pid <- list do
  ...
  def imperfect2DGrid(topology,numNodes, algorithm,nodeNo,maxCount,x,y,map,nodeList,failureCount) when nodeNo<=numNodes do
    ...
    imperfect2DGrid(topology,numNodes, algorithm,nodeNo+1,maxCount,new_x,new_y,map,[current|nodeList],failureCount)
  end
  def imperfect2DGrid(topology,numNodes, algorithm,nodeNo,maxCount,x,y,map,nodeList,failureCount) when nodeNo>numNodes do
    for pid <- nodeList do
  ...
  def killSomeRandomProcesses(x, processList) do
    ...
    for pid <- deathRow do
      IO.puts to_string(:os.system_time(:millisecond))<>"\tNode: "<>(List.to_string(:erlang.pid_to_list(pid)) <> "\thas been killed explicitly")
